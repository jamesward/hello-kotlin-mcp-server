 ...
 */
public class StatelessMcpToolProvider {

	private static final Logger logger = LoggerFactory.getLogger(StatelessMcpToolProvider.class);
	 ...
	 */
	public StatelessMcpToolProvider(List<Object> toolObjects) {
	 ...
	 * methods are found
	 */
	public List<McpStatelessServerFeatures.SyncToolSpecification> getToolSpecifications() {

		List<McpStatelessServerFeatures.SyncToolSpecification> toolSpecs = this.toolObjects.stream()
			.map(toolObject -> Stream.of(doGetClassMethods(toolObject))
				...
				.map(mcpToolMethod -> {
					...
					BiFunction<McpTransportContext, CallToolRequest, CallToolResult> methodCallback = new StatelessMcpToolMethodCallback(returnMode, mcpToolMethod, toolObject);

					var toolSpec = McpStatelessServerFeatures.SyncToolSpecification.builder().tool(tool).callHandler(methodCallback).build();
